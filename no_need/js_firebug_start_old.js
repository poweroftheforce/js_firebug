...
jsfbWindow = open('about:blank', 'jsFirebugWindow', 'toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=600,height=400')
//jsfbWindow.onload = function() { childLoaded(); }
jsfbWindow.document.write('<script>opener.childLoaded();<\/script>');
//jsfbWindow.onreadystatechange = function() {
//	if ( jsfbWindow.readyState == 'loaded' || jsfbWindow.readyState == 'complete' )
//		childLoaded();
//};
function childLoaded() {
	...
	jquery_inc.src = 'http://10.69.68.20/CDEObjects/js_firebug/test_open/jquery.js';
	...
	function jquery_ready() {
		...
		(function($) {
			...
			(function($) {
				$.fn.showMenu = function(options) {
					...
					$(this).bind('contextmenu', function(e){
						...
						$('#htmlContext', jsfbWindow.document).show()
					...
					//$(jsfbWindow.document).bind('click', function(e){
					//	$(opts.query, jsfbWindow.document.body).css('display', 'none');
					//});
				};
			...
			$.extend(jQuery.jsFirebug, {
				prototype: {
					init: function() {
						...
						this.newPropInc = 0;
						this.ffInc = 0;
						this.jsFW = $.data(document.body, 'jsFirebugWindow');
						if ( !this.jsFW ) {
							this.jsFW = new $.jsFirebugWindow();
							$.data(document.body, 'jsFirebugWindow', this.jsFW); 
						};
						/* create the highlight wrap */
						...
						$(this.strNoStyles)
							.attr('id', 'noStylePhrase')
							.text('No Styles');
						this.jsFW.append('Style', $(this.strNoStyles));
						$(document.body)
							...
							if ( obj.nodeType == 1 ) {
								...
								if ( this.currentObj != obj ) {
									...
									if ( this.currentObj.id ) {
										var idAttr = document.createElement('span');
										$(idAttr)
											.addClass('idAttr')
									...
									this.jsFW.append('Style', document.createElement('br') );
									var inheritedFrom = document.createElement('span');
									$(inheritedFrom)
										.addClass('inheritedFrom')
										.text('Inherited from ');
									var oInheritedFromA = document.createElement('a');
									...
									this.jsFW.append('Style', inheritedFrom);
								}
					...
					getHTML: function() {
						...
						$('.htmlLine', jsfbWindow.document)
							...
							.click(function( event ) {
								...
								alert($('#htmlContext', jsfbWindow.document).html())
								$('#htmlContext', jsfbWindow.document).css({
																	   display: 'block',
																	   position: 'absolute',
																	   top: '0px',
																	   left: '0px'
									...
									if ( all[i].id != 'jsFirebugScript' ) {
										if ( $(all[i]).attr('src') ) {
											try {
												...
												this.jsFW.append('Script', headerSpan);
											}
										} else {
											var headerSpan = document.createElement('span');
										...
										this.jsFW.append('Script', dataSpan);
									}
								}
							}
						}
							...
							while ( obj.childNodes.length ) {
								...
								if ( obj.childNodes[k].nodeType == 1 ) {
									...
									if (
										...
										tagName != 'html' &&
										tagName != 'head' ) {
										if ( curObj.id == 'jsfWrap' )
											return;
										var newContainer = document.createElement('div');
										newContainer.style.marginLeft = '20px';
										container.endTag = true;
										var jsFWHtmlString = slash = htmlText = endHTML = '';
										var hasChildren = false;
										var jsFWHtmTagStart = document.createElement("span");
										$(newContainer).append(jsFWHtmTagStart);
										var jsFWHtmlString = '<span class="htmlTag">&lt;' + tagName;
										for ( z=0; z<curObj.attributes.length; z++ ) {
											if ( curObj.attributes[z].name == 'id' || curObj.attributes[z].name == 'class' ) {
												jsFWHtmlString += ' <span class="htmlAttrName">' + curObj.attributes[z].name + '</span>="<span class="htmlAttrValue">' + curObj.attributes[z].value + '</span>"';
											}
										}
										...
										jsFWHtmlString += slash + '&gt;</span>' + htmlText + endHTML;
										jsFWHtmTagStart.obj = curObj;
										$(jsFWHtmTagStart)
											.addClass('htmlLine')
											.html(jsFWHtmlString + '<br />')
											.showMenu({
												opacity	: 0.8,
												query	: '#htmlContext'
											});
										if ( container.endTag ) {
											if ( tagName != 'img' && tagName != 'br') {
												var jsFWHtmTagEnd = document.createElement('span');
												$(jsFWHtmTagEnd)
													.css('padding-left', '7px')
													.addClass('htmlTag')
													.html('&lt;/' + tagName + '&gt;');
												$(newContainer).append(jsFWHtmTagEnd);
					...
					createPropertyValueSet: function( jsFWHtmlWrap, obj, isNew, propW, propVal, propOldVal, valW, valVal, strike, propToChange, image, objToCheck ) {
						...
						$(jsFWHtmlWrap).append(span);
							...
							$(txtVal)
								...
								.keyup(function( event ) {
									$.jsFirebug.changeRuleValue(this, event);
								});
							if (propToChange != '')
					...
					*/
					...
					copyHTML: function() {
						alert($('#htmlContext').get(0).obj.obj.innerHTML)
					}
				}
			});
			...
			$.extend($.jsFirebugWindow, {
				prototype: {
					init: function() {
						...
						this.jsFW = jsfbWindow.document.createElement('div');
						$(this.jsFW).attr('id', 'jsfWrap');
						$(jsfbWindow.document.body).append(this.jsFW);

							this.jsFWMenuWrap = jsfbWindow.document.createElement('div');
							$(this.jsFWMenuWrap).attr('id', 'menuWrap');
							$(this.jsFW).append(this.jsFWMenuWrap);
						...
						*/
						...
						/* setup btnEditHTML */
						$('#btnHTMLCM_CopyHTML').click(function() {
							$.jsFirebug.copyHTML();
						});
					},
					createScreen: function( name, btnId, screenId ) {
						...
						$(btn)
							...
							.html(name);
						$(this.jsFWMenuWrap).append(btn);
						$(btnId, jsfbWindow.document.body)
							.bind('mouseover mouseout', function() {
								$(this).toggleClass('menuBtnHi');
							})
							.click(function() {
								$.jsFirebugWindow.prototype.show(name);
							});
						/* create the Style window */
						var screen = jsfbWindow.document.createElement('div');
						$(screen).attr('id', screenId.substring(1, screenId.length));
						$(this.jsFW).append(screen);
						screen.name = name;
						this.windows.push(screen);
					},
					show: function( scr ) {
						var jsFW = $.data(document.body, 'jsFirebugWindow');
						for ( var i=0; i<jsFW.windows.length; i++ )
							$(jsFW.windows[i]).hide();
						$(jsFW.getWin(scr)).show();
					},
					changeTitle: function( str ) {
						//$(this.jsFWTitle).html(this.title + str);
					},
					getWin: function( scr ) {
						var win = null;
					...
					adjustJSFB: function() {
						var jsFW = $.data(document.body, 'jsFirebugWindow');
						var w = jsfbWindow.outerWidth;
						var h = jsfbWindow.outerHeight;
						$('#jsfWrap', jsfbWindow.document.body).css({
							width	: w + 'px',
							height	: h + 'px'
						});
						$.each(jsFW.windows, function() {
							$(this).css({
								width	: (w - 5) + 'px',
								height	: h + 'px'
							});
			...
			/* startup */
			jsfbWindow.onload = jsfbWindow.onresize = function() {
				$.jsFirebugWindow.prototype.adjustJSFB();
			}
